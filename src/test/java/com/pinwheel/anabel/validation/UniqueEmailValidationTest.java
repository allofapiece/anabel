...
public class UniqueEmailValidationTest {
    ...
    public void shouldRejectIfAlreadyExists() {
        Mockito.doReturn(true).when(userRepository).existsByEmailAndIdNot("test@test.com", 10L);
        ...
        assertTrue(violations.stream().anyMatch(x -> x.getMessageTemplate().contains("email.error.taken")));
    }

    @Test
    public void shouldRejectIfAlreadyExistsWithoutId() {
        Mockito.doReturn(true).when(userRepository).existsByEmail(eq("test@test.com"));
    ...
    public void shouldAccept() {
        ...
        Set<ConstraintViolation<User>> violations = validator.validate(user);
        assertFalse(violations.stream().anyMatch(x -> x.getMessageTemplate().contains("email.error.taken")));
    }
}
